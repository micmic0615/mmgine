...
ai_attack_melee_cooldown_value = 3*SEC;
ai_attack_barrage_cooldown_value = 2*SEC;
ai_attack_strike_cooldown_value = 8*SEC;

ai_attack_extra_cooldown = 0.25*SEC;
		...
		if (ai_attack_strike_active){
			...
			if (action_strike[?"sequence"] == 2){
				if (ai_attack_strike_combo_can_add){
					if (ai_attack_strike_combo_count < 2){
			...
			if (action_shoot[?"sequence"] == 1){
			    ai_attack_melee_combo_can_add = true;
			    ai_attack_melee_combo_can_jump = true
			}
			if (action_shoot[?"sequence"] == 2){
			    if (ai_attack_melee_combo_can_add){
			        if (ai_attack_melee_combo_count < 1){
			            ai_attack_melee_combo_count++;						
			        } else {
			            ladysofia_ai_attack_rest();
			        ...
			        ai_attack_melee_combo_can_add = false;
			    }
			}

			if (action_shoot[?"sequence"] == 3){
			    if (ai_attack_melee_combo_can_jump){
			        entity_sfx_create_pulse(
			            /*sprite*/ ExplosionBulletAlt_idle,
			            /*duration*/ 0.4*SEC,
			            /*blend*/ my_invoke_color_2,
			        ...
			        ai_attack_melee_combo_can_jump = false;
			    }
			}
		} else {
			...
			} else if (ai_attack_strike_cooldown_timer <= 0){
