if (my_clone_can_spawn && my_clone_original ){
	while(my_clone_count > 0){
		var spawned_clone = actor_spawn(x + random(my_clone_spawn_area) - my_clone_spawn_area/2,y + random(my_clone_spawn_area) - my_clone_spawn_area/2,object_index);
		spawned_clone.my_clone_original = false;
		spawned_clone.my_clone_core = id;
		spawned_clone.player_boss = false;
		my_clone_count--;
	}
	
	my_clone_can_spawn = false;
}

if (my_clone_original != true){
	if (instance_exists(my_clone_core)){
		status_health_current = my_clone_core.status_health_current;
	...
	if (my_attack_cooldown_timer <=  0){
		...
		if (my_attack_cast_timer > 0){
			...
			if (floor_age != next_floor_age && floor_age % (0.05*SEC) == 0){
				entity_mirage_create(0.4*SEC, 0, 0, make_color_rgb(255,0,255));
			}
		}
	}
	
	if (my_attack_channel_ongoing > 0){
		if (floor_age != next_floor_age && floor_age % (0.75*SEC) == 0){
			...
			bullet.explosion_lifespan_base = 1*SEC;			
