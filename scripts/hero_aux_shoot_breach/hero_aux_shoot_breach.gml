for(var i = 0; i < ds_list_size(my_shoot_aux_main);i++){
	...
	switch(p){
		...
		case "split":
			my_shoot_bullet_radius *= 0.75;
			my_shoot_bullet_count += 2;
			action_shoot_bullet_seek_range *= 0.7;
			action_shoot_bullet_seek_turn_rate *= 0.8;
			action_shoot_angle_spead = 160;
			
			my_shoot_damage = [
				(my_shoot_damage[0] * 0.45),
				(my_shoot_damage[1] * 0.45),
			];
			
			my_shoot_flinch = [
				(my_shoot_flinch[0] * 0.4),
				(my_shoot_flinch[1] * 0.4),
			];
			break;
