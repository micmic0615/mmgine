...
if (my_attack_cooldown_timer <= 0){
	...
	actor_buff_apply("speed_bonus_raw", 1.5*SEC, [600*PPS], "mana_speed_boost")
	entity_motion_push(my_attack_dash_range*channel_multiplier_dash, (my_attack_dash_speed/(my_attack_dash_range*PPS))*SEC, bullet.bullet_action_move_angle - 180, ["multiply",1.25], "move_motion")
	
	my_attack_channel_power_current = 0;
	my_attack_cooldown_timer = my_attack_cooldown_value;
} else {
	my_attack_queue = target_point;
}