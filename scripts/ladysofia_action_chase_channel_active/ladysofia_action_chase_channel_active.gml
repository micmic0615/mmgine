...
if (my_chase_dash_timer <= 0){
	...
	var rad_angle = degtorad(angle_between(action_chase[?"target_entity"].x,action_chase[?"target_entity"].y,x,y) + random_mirror(90));
	if (distance_between(x,y,action_chase[?"target_entity"].x,action_chase[?"target_entity"].y) < action_dash[?"range"]*1.5){
		rad_angle = degtorad(angle_between(action_chase[?"target_entity"].x,action_chase[?"target_entity"].y,x,y));
	}
