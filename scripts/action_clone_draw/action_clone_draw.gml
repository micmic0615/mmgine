...
if (actor_clone_draw_timer > 0 || is_flinched){
	actor_clone_draw_timer -= TIMESPEED;
	
	var family_length = ds_list_size(actor_clone_family);
	var sibling = noone;
	for(var i = 0; i < family_length;i++){
		var p = ds_list_find_value(actor_clone_family, i);
		if (p == id){
			if (i+1 < family_length){
				sibling = ds_list_find_value(actor_clone_family, i+1);
			} else {
				sibling = ds_list_find_value(actor_clone_family, 0);
			}
		}
	};

	if (sibling != noone){
		var draw_time =  is_flinched ? 2 : (actor_clone_draw_timer/(0.5*SEC))
